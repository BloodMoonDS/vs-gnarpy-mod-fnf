function onCreate()
	--Iterate over all notes
	for i = 0, getProperty('unspawnNotes.length')-1 do
		--Check if the note is an Instakill Note
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Poob Sing' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'NOTE_assets'); --Change texture
			setPropertyFromGroup('unspawnNotes', i, 'hitHealth', '0.023'); --Default value is: 0.023, health gained on hit
			setPropertyFromGroup('unspawnNotes', i, 'missHealth', '0.0475'); --Default value is: 0.0475, health lost on miss
			setPropertyFromGroup('unspawnNotes', i, 'hitCausesMiss', false);

			if getPropertyFromGroup('unspawnNotes', i, 'mustPress') then --Doesn't let Dad/Opponent notes get ignored
				setPropertyFromGroup('unspawnNotes', i, 'ignoreNote', false); --Miss has no penalties
			end
		end
	end
	--debugPrint('Script started!')
end


-- Function called when you hit a note (after note hit calculations)
-- id: The note member id, you can get whatever variable you want from this note, example: "getPropertyFromGroup('notes', id, 'strumTime')"
-- noteData: 0 = Left, 1 = Down, 2 = Up, 3 = Right
-- noteType: The note type string/tag
-- isSustainNote: If it's a hold note, can be either true or false
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'Poob Sing' then
		--debugPrint(id)
		characterPlayAnim("bf", "idle", true)
		if id == 0 then
			characterPlayAnim("gf", "singLEFT", false)
		end
		if id == 1 then
			characterPlayAnim("gf", "singDOWN", false)
		end
		if id == 2 then
			characterPlayAnim("gf", "singUP", false)
		end
		if id == 3 then
			characterPlayAnim("gf", "singRIGHT", false)
		end
		if id == 4 then
			characterPlayAnim("gf", "singLEFT", false)
		end
		if id == 5 then
			characterPlayAnim("gf", "singDOWN", false)
		end
		if id == 6 then
			characterPlayAnim("gf", "singUP", false)
		end
		if id == 7 then
			characterPlayAnim("gf", "singRIGHT", false)
		end
		
		if getPropertyFromGroup('unspawnNotes', id, 'hitByOpponent') == true then
			cameraShake('game', 0.01, 0.1);
			
		end
		-- put something here if you want
	end
end

-- Called after the note miss calculations
-- Player missed a note by letting it go offscreen
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'Poob Sing' then
		cameraShake('game', 0.01, 0.1);
		-- put something here if you want
	end
end

function opponentNoteHit(id, direction, noteType, isSustainNote)
	if noteType == 'Poob Sing' then
		characterPlayAnim("dad", "idle", true)
		if id == 0 then
			characterPlayAnim("gf", "singLEFT", false)
		end
		if id == 1 then
			characterPlayAnim("gf", "singDOWN", false)
		end
		if id == 2 then
			characterPlayAnim("gf", "singUP", false)
		end
		if id == 3 then
			characterPlayAnim("gf", "singRIGHT", false)
		end

		if id == 4 then
			characterPlayAnim("gf", "singLEFT", false)
		end
		if id == 5 then
			characterPlayAnim("gf", "singDOWN", false)
		end
		if id == 6 then
			characterPlayAnim("gf", "singUP", false)
		end
		if id == 7 then
			characterPlayAnim("gf", "singRIGHT", false)
		end
		
		if getPropertyFromGroup('unspawnNotes', id, 'hitByOpponent') == true then
			cameraShake('game', 0.01, 0.1);
			
		end
		-- put something here if you want
	end
	-- Works the same as goodNoteHitPost, but for Opponent's notes being hit
	-- Values work the same as goodNoteHitPre
end