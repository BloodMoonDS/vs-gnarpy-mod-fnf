...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "setLuaSpriteScrollFactor", function(obj:String, scrollX:Float, scrollY:Float) {
			if(PlayState.instance.getLuaObject(obj,false)!=null) {
				PlayState.instance.getLuaObject(obj,false).scrollFactor.set(scrollX, scrollY);
				luaTrace('setLuaSpriteScrollFactor is deprecated using SetScrollFactor instead' + obj, false, false, FlxColor.YELLOW);
				return;
			}
		...
		Lua_helper.add_callback(lua, "scaleLuaSprite", function(obj:String, x:Float, y:Float, updateHitbox:Bool = true) {
			if(PlayState.instance.getLuaObject(obj)!=null) {
				var shit:FlxSprite = PlayState.instance.getLuaObject(obj);
				shit.scale.set(x, y);
				if(updateHitbox) shit.updateHitbox();
				luaTrace('scaleLuaSprite is deprecated, using scaleObject Instead', false, false, FlxColor.YELLOW);
				return;
			}
			...
			luaTrace('scaleLuaSprite: Couldnt find object: ' + obj, false, false, FlxColor.RED);
			luaTrace('scaleLuaSprite is deprecated, using scaleObject Instead', false, false, FlxColor.YELLOW);
		});
		...
		Lua_helper.add_callback(lua, "setLuaSpriteScrollFactor", function(tag:String, scrollX:Float, scrollY:Float) {
			...
			luaTrace('setLuaSpriteScrollFactor is deprecated use setScrollFactor Instead' + '' + '' + '' + '' + '', true, false);
			return false;
		});
